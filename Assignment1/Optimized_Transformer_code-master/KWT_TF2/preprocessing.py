...
def create_example(fp, commands, data_dir, trainable=True):
    ...
    patches = get_patches(mfccs)
    return patches, label_id
...
def collect_data(batch_size, data_dir):
    ...
    # np.save('train_data.txt', train_data)
    # np.save('train_label.txt', train_label)
    # np.save('test_data.txt', test_data)
    # np.save('test_label.txt', test_label)
    # np.save('val_data.txt', val_data)
    # np.save('val_label.txt', val_label)
    # exit(0)

    # commands = ['down', 'go', 'left', 'no', 'off', 'on', 'right', 'stop', 'up', 'yes']
    # train_data = np.load('train_data.txt.npy')
    # train_label = np.load('train_label.txt.npy')
    # test_data = np.load('test_data.txt.npy')
    # test_label = np.load('test_label.txt.npy')
    # val_data = np.load('val_data.txt.npy')
    # val_label = np.load('val_label.txt.npy')
